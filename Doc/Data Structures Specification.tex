...
\paragraph*{Comments}
This is a structure whose implementation depends a lot on the data storage backend. On RDBMSs it is straightforward to implement it, but NoSQL backends can only implement a simplified version of it, where the garbage-collection is done by the server. The following is specified with RDBMSs in mind. For NoSQL the specification is used to define the functionality of the garbage collector.
...
\paragraph*{Comments}
A HashBuffer is a hash with the algorithm used to produce it. The algorithm acoompanies the buffer for forward-compatibility.
...
\subsubsection{AssymetricKeyAlgorithm}
\begin{verbbox}
AssymetricKeyAlgorithm = { ECDSA-prime192v1 }
\end{verbbox}
\begin{center}
\theverbbox
\end{center}
...
\paragraph*{Comments}
AssymetricKeyAlgorithm is an enum of all the acceptable assymetric key generation algorithms in the system. For the moment only ECDSA-prime192v1 is accepted.
...
{
  buffer, bytearray
  algorithm, AssymetricKeyAlgorithm
}
\end{verbbox}
\begin{center}
\theverbbox
...
\paragraph*{Comments}
An EncryptedBuffer is a ciphertext payload with the algorithm used to produce it. The algorithm acoompanies the buffer for forward-compatibility.
...
PublicKey
{
  buffer, bytearray with X.509 encoding of the key
  algorithm, AssymetricKeyAlgorithm
}
\end{verbbox}
\begin{center}
...
\paragraph*{Comments}
A PublicKey is a buffer with the X.509 encoding of the key with the algorithm used to produce it. The algorithm acoompanies the buffer for forward-compatibility.
...
PrivateKey
{
  buffer, bytearray with PKCS#8 encoding of the key
  algorithm, AssymetricKeyAlgorithm
}
\end{verbbox}
\begin{center}
...
\paragraph*{Comments}
A PrivateKey is a buffer with the PKCS\#8 encoding of the key with the algorithm used to produce it. The algorithm acoompanies the buffer for forward-compatibility.
...
\begin{verbbox}
HashedContact = Common.HashedContact
\end{verbbox}
\begin{center}
\theverbbox
\end{center}
...
\paragraph*{Comments}
Server-side typedef for defining the foreign key constraint.
...
\begin{verbbox}
Hashes = map(HashBuffer:ContactNode)
\end{verbbox}
\begin{center}
\theverbbox
\end{center}
