...
\begin{description}
 ...
 \item[\userIdSimple{A} : Id] A user id \identity{A} generated by \Server{}.
...
\userIdSimple{A} and adds $U(A)$ to the UserSet.Identities for \userIdSimple{A}.
 ...
 \item \Client{A} populates \signedData{} from \signingKey{} and \identity{B}, \publicKeys{}, $U(A)$, $U_{id}(A)$, $\lbrace U_1(A),\ldots,U_n(A)\rbrace$, $j$, where $j$ is the 
index for \identity{A} in the set of identities.
 ...
 \end{enumerate}
 \item \Server{} registers $\lbrace U_1(A),\ldots,U_n(A)\rbrace$ for $U_{id}(A)$ in HashIdentityMap and UserSet.
...
\begin{enumerate}[resume]
 ...
 \begin{enumerate}
  ...
  \item \Client{B} populates \signedData{} from \signingKey{} and $U'(C)$.
...
\SpecialItem
\begin{description}
 \item[\userIdSimple{A} : Id] The user id of $U(A)$.
 \item[\userIdSimple{B} : Id] The user id of $U(B)$.
...
\begin{enumerate}
 ...
 \end{enumerate}
 \item \Server{} populates \userIdSimple{B} from HashIdentityMap for $U(B)$.
 \item \Server{} populates \userIdSimple{A} from HashIdentityMap for $U(A)$.
