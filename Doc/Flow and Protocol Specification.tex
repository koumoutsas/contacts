...
\newcommand{\publicKeys}[1]{\parenthesize{publicKeys}{#1}}
\newcommand{\signingKey}{\privateKeys{}.\emph{signing}}
\newcommand{\verificationKey}{\publicKeys{}.\emph{verification}}
\newcommand{\encryptionKey}[1]{\publicKeys{#1}.\emph{encryption}}
...
\begin{enumerate}[resume]
 \item \Client{A} calculates $i=$\identity{C}$^{-1}$ for \encryptionKey{B} and sets $I=E(A,E(B,i\odot R))$. The inverse is calculated according to the homomorphism of the crypto 
system, denoted with $\odot$. For RSA, for example, it is multiplication.
 ...
 \begin{enumerate}
  \item \Server{} calculates $E(A,i)\odot I$ and compares it to $I_R$. If they are equal the contacts match.
 \end{enumerate}
