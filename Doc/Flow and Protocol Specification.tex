...
\newcommand{\publicKeysSet}[1]{\parenthesize{publicKeysSet}{#1}}
...
All flows involving \Server{} and another entity may fail for various reasons, such as \userId{A} or \clientId{A} being unknown or the signature verification failing. For security, 
...
\begin{description}
 ...
 \item[\parenthesize{Id}{A} : Server.IdentityId] A pair of user id and identity id for \identity{A} generated by \Server{}.
...
\begin{enumerate}
 ...
 \begin{enumerate}
  \item If there is, it populates $U_{id}(A)$ with the value of the mapping.
  \item If there isn't, it generates a new $U_{id}{A}$ and creates a mapping from $U(A)$ to $U_{id}(A)$ in HashIdentityMap. It creates an entry in UserMap with $U_{id}(A).userId$ and adds $U(A)$ to the UserMap.Identities for $U_{id}(A).identityId$.
 \end{enumerate}
 \item \Server{} adds \Client{B} to the list of confirmers of $U_{id}(A).identityId$ in UserMap.
 \item \Server{} populates $N_S(A)$ and sets UserMap.Blind for $U_{id}(A)$ to it.
 ...
 \item \Server{} retrieves $N_S(A)$ from UserMap.Blind and key $U_{id}(A)$.
 \begin{enumerate}
 ...
 \item \Client{A} populates \signedData{} from \signingKey{} and \identity{B}, \publicKeys{}, $U(A)$, $U_{id}(A)$, $\lbrace U_1(A),\ldots,U_n(A)\rbrace$, $j$, where $j$ is the index for \identity{A} in the set of identities.
 ...
 \end{enumerate}
 \item \Server{} registers $\lbrace U_1(A),\ldots,U_n(A)\rbrace$ for $U_{id}(A)$ in HashIdentityMap and UserMap.
...
\begin{description}
 ...
 \item[\signedData{} : ECDSA signature] The ECDSA signature with \signingKey{} of $U_R$. See \cite{crypto_spec} for details.
\end{description}
...
\begin{enumerate}
 ...
 \item \Server{} stores $U_N$ to UserMap.Identities for \userId{A}.
...
\SpecialItem
\begin{description}
 \item[\userId{A} : Id] The user id of \Client{A}, known by \Server{}.
 \item[\clientId{A} : Id] The client id of \Client{A}, known by \Server{}.
 \item[\userId{B} : Id] The user id of \Client{B}, known by \Server{}.
 \item[\clientId{B} : Id] The client id of \Client{B}, known by \Server{}.
 \item[\privateKeys{} : PrivateKeys] The set of private keys.
 \item[\encryptedData{} : AsymmetricCryptoBuffer] Encrypted payload exchanged between \Client{A} and \Client{B}. See \cite{crypto_spec} for details.
 \item[\signedData{} : ECDSA signature] The ECDSA signature of \encryptedData{} with \signingKey{}. See \cite{crypto_spec} for details.
 \item[\publicKeys{} : PublicKeys] The set of public keys.
 \item[R : RandomHashPad] A random blind for \identity{C}.
 \item[\publicKeysSet{B}] The set of public keys for all clients of \userId{B}. Generated by \Server{}.
 \item[I : AsymmetricCryptoBuffer] A double encryption of \identity{C}
 \item[$I_R$ : AsymmetricCryptoBuffer] A double encryption of $R$.
\end{description}

\paragraph{Flow}

\begin{enumerate}
 \item \Client{A} sends to \Server{} request for contact comparison for \userId{B}.
 \item \Server{} populates \publicKeysSet{B}.
 \item \Server{} sends to \Client{A} \publicKeysSet{B}.
 \item \Client{A} populates \privateKeys{} from local storage.
 \item \Client{A} generates $R$.
\end{enumerate}
The following is executed for each member of \publicKeysSet{B} and each \Client{B} corresponding to this member. Each communication round between \Client{A} and \Server{} 
...
system, denoted with $\odot$. For RSA it is multiplication.
 ...
 \item \Server{} populates \publicKeys{} from UserMap and \userId{A} and \clientId{a}.
 \item \Server{} verifies \signedData{}.
 \item \Server{} retrieves ComparisonIdentities from UserMap for \userId{B} and \clientId{B} and for each member $i$
 \begin{enumerate}
  \item It calculates $i\odot I$ and compares it to $I_R$. If they are equal the contacts match.
 \end{enumerate}
 \item If there is a match, \Server{} sends to \Client{A} a notification with \userId{B}, \clientId{B}, otherwise, the flow terminates.
 \item \Server{} notifies \Client{A} with contact comparison response \userId{B}, \clientId{B}, \signedData{}, and \publicKeys{}.
\end{enumerate}
...
\begin{enumerate}[resume]
 ...
 \item \Client{A} sends to \Server{} \signedData{}, \userId{B}, \clientId{B}.
 \item \Server{} populates \publicKeys{} from \userId{A}, \clientId{A}.
 \item \Server{} verifies \signedData{}.
 \item \Server{} sends to \Client{B} \signingKey{}, \signedData{}.
 ...
 \item If the mapping doesn't exist already, \Client{B} executes the flow for all its contacts.
...
\SpecialItem
\begin{description}
 \item[\userId{A} : Id] From \Client{A} local storage.
 \item[\clientId{A} : Id] From \Client{A} local storage.
...
\begin{enumerate}
 ...
 \item \Client{A} populates \privateKeys{} from local storage.
 \begin{enumerate}
  \item For each $c$ in \addedContacts{}, \Client{A} adds to \contactOperationSet{} a ContactOperation with type $Add$, contact $c.hash$, and comparisonIdentity $E(A,c.target)$. Note that $c.hash$ may be empty for those contacts for which the hash is not known yet.
  \item For each $c$ in \deletedContacts{}, \Client{A} adds to \contactOperationSet{} a ContactOperation with type $Delete$, contact $c.hash$, and comparisonIdentity $E(A,c.target)$. Note that $c.hash$ may be empty for those contacts for which the hash is not known yet.
  \item For each $c$ in \updatedContacts{}, \Client{A} adds to \contactOperationSet{} a ContactOperation with type $Update$, contact $c.hash$, and empty comparisonIdentity. Note that $c.hash$ cannot be empty for these contacts, since the update operation is executed only for these contacts for which the hash became known through \ref{broadcast_new_contact_identity_flow}.
 \end{enumerate}
 \item \Client{A} generates \signedData{} from \signingKey{} and \contactOperationSet{}.
 \item \Client{A} sends \signedData{}, \userId{A} and \clientId{A} to \Server{}.
 ...
 \begin{enumerate}
  ...
  \begin{enumerate}
   \item \Server{} adds $a.comparisonIdentity$ to UserMap.ComparisonIdentities for \userId{A} and \clientId{A}. If the entry exists, the flow terminates with error.
   \item \Server{} updates the graph and generates the appropriate notifications to \Client{A}.
  \end{enumerate}
  \item If the operation is {\em Delete}
  \begin{enumerate}
   \item \Server{} deletes $a.comparisonIdentity$ from UserMap.ComparisonIdentities for \userId{A} and \clientId{A}. If the entry doesn't exist, the flow terminates with error.
...
\SpecialItem
\begin{description}
 \item[U : HashBuffer] The blinded identity of \User{B} known to \Client{B}.
 \item[\privateKeys{} : PrivateKeys] The private keys of the client.
 \item[\publicKeys{} : PublicKeys] The public keys of the client.
 \item[\signedData{} : AsymmetricCryptoBuffer] The signed data to be sent from a client.
 \item[\encryptedData{} : AsymmetricCryptoBuffer] The encrypted contact card.
...
\begin{enumerate}
 \item \User{A} requests the contact card of \User{B}. This may happen by \User{A} selecting \User{B} from the contact book of \Client{A} or by \Server{} sending a suggestion to \Client{A} for \User{B} in flow \ref{suggest_new_contact_flow}.
...
\end{enumerate}
For the rest of the flow, we assume that there is only one client for \User{B}. If there are more, \Server{} executes \ref{send_contact_card_flow:one} for all clients for \userId{B} and the rest of the flow is executed for each one that responds. If more than one contact cards reach \Client{A}, they are merged in the native and \longProduct{} contact books.
\begin{enumerate}[resume]
...
\SpecialItem
\begin{description}
 \item[U(A) : HashBuffer] The blinded identity of the receiver of the contact suggestion.
 \item[U(B) : HashBuffer] The blinded identity of the suggested contact.
\end{description}

\paragraph{Other Variables}
\SpecialItem
\begin{description}
 \item[\userId{A} : Id] The user id of $U(A)$.
 \item[\userId{B} : Id] The user id of $U(B)$.
 \item[\clientId{B} : Id] The client id of \userId{B}
...
\begin{enumerate}
 \item \Server{} checks if $U(B)$ is in UserMap.SentRequests for \userId{A}.
 \begin{enumerate}
  \item If it is, the flow terminates.
 \end{enumerate}
 \item \Server{} populates \userId{B} from HashIdentityMap for $U(B)$.
 \item \Server{} populates \userId{A} from HashIdentityMap for $U(A)$.
 \item For all \Client{A} in UserMap for \userId{A}
 \begin{enumerate}
  \item \Server{} sends \userId{B}, $U(B)$, \encryptionKey{} for all clients for \userId{B}.
  ...
  \item \Server{} verifies the signature and checks if that's the first reply from \Client{A} for $U(B)$ in UserMap.SentRequests.
  \begin{enumerate}
   \item If it is not, the flow terminates.
   \item If it is, it enters it in UserMap.SentRequests.
  \end{enumerate}  
 \end{enumerate}
